...
public class KpiService {
        ...
        /**
         * Adds a KPI object to the model containing the k-anonymity value for the given
         * anonymization object.
         * 
         * @param model               Model to which the KPI object is added
         * @param anonymizationObject Anonymization object for which the KPI object is
         *                            created
        ...
        public static void addKpiObject(
                ...
                kpiObject.addLiteral(kAnonymity,
                                calculateKAnonymity(model, anonymizationObject, attributes, configurations));
        }

        /**
         * Adds the number of buckets used in the anonymization for an attribute to the
         * KPI object in the model.
         * 
         * @param model               Model to which the KPI object is added
                ...
                attributes.stream().filter(attr -> configurations.get(attr).getAnonymization().equals("randomization"))
                                .forEach(randomization -> {
                                        ...
                                        similarValues.keySet().forEach(
                                                        resource -> similarValues.get(resource)
                                                                        .retainAll(similarity.get(resource)));
                                });
        ...
        private static Map<Resource, Set<Resource>> getSimilarValues(
                        ...
                        Property property,
                        boolean date) {
                List<QueryService.RandomizationResult> randomizationResults = QueryService
                                .getRandomizationResults(model, resource, property);
                        ...
                        if (randomization.original() != null) {
                                if (date) {
                                        distances.add(Math.abs(
                                                        RandomizationDateTime.literalToNumericDate(
                                        ...
                                        originalValuesMap.computeIfAbsent(
                                                        RandomizationDateTime.literalToNumericDate(
                                                                        randomization.original()),
                                                        _ -> new HashSet<>()).add(randomization.object());
                                } else {
                                        distances.add(Math.abs(
                                                        randomization.original().getDouble()
                                                                        - randomization.randomized().getDouble()));
                                        randomizedData.put(randomization.object(),
                                                        randomization.randomized().getDouble());
                                        originalValuesMap.computeIfAbsent(
                ...
                if (!distances.isEmpty()) {
                        double benchmark = distances.stream().mapToDouble(Double::doubleValue).sum() * 2
                                        / distances.size();
                        randomizedData.forEach((key, value) -> similarity.put(key,
                                        findInRange(value, benchmark, originalValuesMap)));
                }
        ...
        public static Set<Resource> findInRange(
                ...
                NavigableMap<Double, Set<Resource>> sub = originalValuesMap.subMap(minInclusive, true, maxInclusive,
                                true);
